...
const buscador= async(x)=>{
    ...
    try{
        ...
        let res= await fetch(`https://remotive.io/api/remote-jobs?limit=5&company_name=${x}`)
        let datos= await res.json()
        let jobs= await datos.jobs
        console.log(datos.jobs)
        // console.log('dea')
...
const trabajosfulltime=async (x) =>{
    ...
    try{
        console.log('entro al buscador')
        let res= await fetch(`https://remotive.io/api/remote-jobs?limit=5&job_type=full_time&company_name=${x}`)
        let datos= await res.json()
        let jobs= await datos.jobs
        console.log(datos.jobs)
        // console.log('dea')
...
inputubi.addEventListener('change',(e)=>{
    let z=e.target.value
    console.log(z,typeof(z),z.length)
    if(e.target.value.length>0){
        borrarhijosubi()
